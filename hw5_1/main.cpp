...
void DeterDist() {
    ...
    while (i) {
        ...
        if (abs(int(DIST2PATTERN - len)) <= 1) {                     // int(d_temp) < len // abs(int((original_ang + ang_step*(SAMPLE-i)) - ang_temp)) <= 2 && //  && abs(DIST2PATTERN - len) <= 2
            angles[i-1] = ang_temp;     
            dist[i-1] = d_temp;
        }
        //ThisThread::sleep_for(20ms);
        if (abs(int((original_ang + ang_step*(SAMPLE-i)) - ang_temp)) <= 5)
            i = i - 1;
    }
